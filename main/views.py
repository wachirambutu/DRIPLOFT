...
def chatbot(request):
    	return render(request, 'main/chatbot.htm')
...
# 			print(order)
			if pay_mode == 'on':
				...
				if int(new_prod) in trends:
				    t = trend.objects.filter(product = Product.objects.filter(product_id=int(new_prod)).first())[0];t.number += 1;t.save();
				else:
...
# 			print(order)
	...
	if temp == 1:
		...
		for item in cart_prods:
			...
			if int(item.product_id) in trends:
			    t = trend.objects.filter(product = Product.objects.filter(product_id=int(item.product_id)).first())[0]; t.number += 1; t.save();
			else:
